...
def get_csv_headers(bucket: str, file_name: str, delimiter: str = ",") -> List[str]:
    ...
    try:
        ...
        handle_failure(
            ...
            file_name,
            "SKIPPED",
            f"Could not read headers from file: {e}",
            trace_id,
            ingestion_id,
        )
...
def process_file(cloud_event):
    ...
    try:
        ...
        log_event(
            "INFO",
            f"✅ Successfully inserted {row_count} records into table: {final_table_ref}.",
            trace_id,
        )
...
def load_raw_strings(
    ...
    try:
        ...
        log_event(
            "INFO",
            f"⏳ Loading to staging with STRING schema: {staging_table_id}",
            trace_id,
        )
        ...
        try:
            ...
            log_event(
                "WARNING",
                "Target table missing. Creating from staging schema...",
                trace_id,
            )
        ...
        if new_columns:
            log_event(
                "WARNING",
                f"⚠️ Schema Drift: Adding {len(new_columns)} STRING columns.",
                trace_id,
            )
...
def handle_failure(bucket, file_name, status, error_msg, trace_id, ingestion_id):
    try:
        # 1. Archive to 'exempted' folder
        archive_file(bucket, file_name, "exempted", trace_id)

        # 2. FIX: Construct the missing file_uri
        file_uri = f"gs://{bucket}/{file_name}"

        # 3. FIX: Pass arguments in the correct order matches your definition
        # Definition: trace_id, ingestion_id, file_name, file_uri, status, start_time
        audit_log(
            trace_id,
            ingestion_id,
            file_name,
        ...
        # If even the error handler fails, just print it so we don't crash-loop
        print(f"CRITICAL ERROR in handle_failure: {e}")
